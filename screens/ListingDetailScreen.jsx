...
var map = L.map('map', {
  zoom: 10,
  maxZoom: 18,
  center: [${listingData?.contact?.latitude || config?.map?.center?.lat || 0
    }, ${listingData?.contact?.longitude || config?.map?.center?.lng || 0}],
  layers: [osmLayer],
  dragging:false
});

const marker = L.marker([${listingData?.contact?.latitude || config?.map?.center?.lat || 0
    }, ${listingData?.contact?.longitude || config?.map?.center?.lng || 0
              ...
              {admobConfig.admobEnabled && (
                <View
                  style={{
                    alignItems: "center",
                    justifyContent: "center",
                    minHeight: isAdLoaded == true ? 100 : 0,
                    marginBottom: 20,
                  }}
                >
                  <AdmobBanner
